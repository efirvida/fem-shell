...
class StaticLinearSolver(Solver):
    """
    Linear static solver using PETSc KSP (CG + GAMG) via Rust.

    Solves K·u = F. Assembly and the KSP solve run entirely in Rust/PETSc.
    The Python layer only orchestrates BCs and forwards COO triplets to
    ...
    def _build_f_ext(self) -> np.ndarray:
        ...
        for load in self.nodal_loads:
            ...
            if len(dofs) != len(vals):
                raise ValueError(
                    f"Nodal load dof/value length mismatch: {len(dofs)} != {len(vals)}"
                )
            f[dofs] += vals
