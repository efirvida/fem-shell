...
class KeyPoints:
    ...
    def generate(self, definition: Definition, geometry: Geometry):
        ...
        for k in range(num_istations):
            ...
            d = np.amin(((geometry.arclength[0, k] + n2), 0.85 * geometry.arclength[ns, k]))
            ...
            d = np.amax((geometry.arclength[-1, k] - n2, 0.85 * geometry.arclength[nf, k]))
        ...
        for ksw in range(max(component_groups)):  # for each shear web
            ...
            if hp["pt"]:
                ...
                for k in range(num_istations):
                    self.web_cpos[ksw][0, k] = interpolator_wrap(k_arclen, k_cpos, p[k])
                    self.web_points[ksw][0, :, k] = interpolator_wrap(k_arclen, k_geom, p[k])
            elif hp["pt3"]:
                try:
                ...
                for k in range(num_istations):
                    ...
                    self.web_points[ksw][0, :, k] = interpolator_wrap(k_cpos, k_geom, p[k])
            else:
                raise Exception(
            ...
            if lp["pt"]:
                ...
                for k in range(num_istations):
                    self.web_cpos[ksw][1, k] = interpolator_wrap(k_arclen, k_cpos, p[k])
                    self.web_points[ksw][1, :, k] = interpolator_wrap(k_arclen, k_geom, p[k])
            elif lp["pt3"]:
                try:
                ...
                for k in range(num_istations):
                    self.web_arcs[ksw][1, k] = interpolator_wrap(k_cpos, k_arclen, p[k])
                    self.web_points[ksw][1, :, k] = interpolator_wrap(k_cpos, k_geom, p[k])
            else:
                raise Exception(
        ...
        for kc in range(num_istations - 1):
            for kr in range(num_areas):
                ...
                ibarc = np.linspace(self.key_arcs[kr, kc], self.key_arcs[kr + 1, kc], npts)
