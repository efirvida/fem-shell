    ...
    def generate(self, definition: Definition, keypoints: KeyPoints):
        ...
        for comp_name in outer_shape_comps:
            ...
            for k_layer in range(1, int(np.max(num_layers)) + 1):
                begin_station, end_station = find_layer_extents(num_layers, k_layer)
                ...
                for ks in range(ks_max):
                    ...
                    if hpRegion:
                        ...
                        arcs = (
                            keypoints.key_arcs[hpRegion[1], begin_station[ks] : end_station[ks] + 1]
                            - keypoints.key_arcs[
                        ...
                        self.indices["hp"].append([begin_station[ks], end_station[ks], *hpRegion])
                    ...
                    if lpRegion:
                        ...
                        regionarea = sum(areas.flatten())
                        arcs = (
                            keypoints.key_arcs[lpRegion[1], begin_station[ks] : end_station[ks] + 1]
                            - keypoints.key_arcs[
                        ...
                        cur_bom.thickness = mat.layerthickness
                        cur_bom.weight = mat.drydensity * regionarea
                        self.indices["lp"].append([begin_station[ks], end_station[ks], *lpRegion])
        ...
        for comp in sw_comps:
            ...
            for k_layer in range(1, int(np.max(num_layers)) + 1):
                ...
                for ks in range(ks_max):
                    ...
                    sw_begin_station[swnum] = np.amin([*begin_station, sw_begin_station[swnum]])
                    sw_end_station[swnum] = np.amax([*end_station, sw_end_station[swnum]])
                    areas = keypoints.web_areas[swnum][begin_station[ks] : end_station[ks]]
                    regionarea = sum(areas.flatten())
                    cur_bom = BillOfMaterialsEntry()
                    ...
                    self.indices["sw"][swnum].append([begin_station[ks], end_station[ks]])
                    swrow = swrow + 1
