...
class Blade:
    ...
    def updateGeometry(self):
        ...
        self.cpos = np.concatenate(
            (-ic[-1, :].reshape(1, -1), -np.flipud(ic), ic[1:, :], ic[-1, :].reshape(1, -1)), axis=0
        )
        ...
        for k in range(N):
            ...
            self.LParcx0[0, k] = (
                interpolator_wrap(xx[-2 : LE - 1 : -1], arclen[-2 : LE - 1 : -1], 0) - LEarcsum
            )
    ...
    def updateKeypoints(self):
        ...
        self.keylabels = ["te", "e", "d", "c", "b", "a", "le", "a", "b", "c", "d", "e", "te"]
            ...
            if hp["pt"]:
                ...
                for k in range(N):
                    self.webarcs[ksw][0, k] = interpolator_wrap(
                        self.cpos[ns : nf + 1, :, k], self.arclength[ns : nf + 1, :, k], p[k]
                    )
        ...
        for kc in range(N - 1):
            for kr in range(M):
                ...
                ib = interpolator_wrap(
                    self.arclength[ns : nf + 1, kc], self.geometry[ns : nf + 1, :, kc], ibarc
                )  # inboard xyz
