...
def excel_to_blade(blade, filename=None):
    ...
    blade.degreestwist = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] : lastrow, xls_dict["geom"]["twist"]], dtype=float
    )
    blade.chord = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] : lastrow, xls_dict["geom"]["chord"]], dtype=float
    )
    blade.percentthick = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] : lastrow, xls_dict["geom"]["thick"]], dtype=float
    )
    blade.chordoffset = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] : lastrow, xls_dict["geom"]["offset"]], dtype=float
    )
    blade.aerocenter = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] : lastrow, xls_dict["geom"]["aerocenter"]],
        dtype=float,
    )
    ...
    afspan = np.array(
        num.iloc[xls_dict["geom"]["datarow1"] :, xls_dict["geom"]["afspan"]], dtype=float
    )
    ...
    for k in range(len(afspan)):
        if afspan[k] < np.amin(blade.span) or afspan[k] > np.amax(blade.span):
            raise Exception(
                "xlsBlade: location of airfoil #%d is outside given span distribution", k
            )
    ...
    for k in range(N):
        ...
        comp.cp = np.stack(
            (
                comp.cp,
                readnumlist(raw.iloc[xls_dict["cmpt"]["datarow1"] + k, xls_dict["cmpt"]["cpnlay"]]),
            ),
            axis=1,
        )
    ...
    for k in range(N):
        ...
        if mat.type == "orthotropic":
            ...
            mat.gxy = (
                MPa_to_Pa * num.iloc[xls_dict["mtrl"]["datarow1"] + k, xls_dict["mtrl"]["gxy"]]
            )
            mat.gyz = (
                MPa_to_Pa * num.iloc[xls_dict["mtrl"]["datarow1"] + k, xls_dict["mtrl"]["gyz"]]
            )
            mat.gxz = (
                MPa_to_Pa * num.iloc[xls_dict["mtrl"]["datarow1"] + k, xls_dict["mtrl"]["gxz"]]
            )
