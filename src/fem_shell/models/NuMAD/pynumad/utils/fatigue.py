...
def calcFatigue(
    ...
    if numElem == 0:
        if iSegment != 13:
            raise Exception(
                '"zwidth" is smaller than element size. Decrease element size or increase zwdith'
            )
        else:
    ...
    SFf = SFs
    warnings.warn(
        "SFf = SFs for BAR project since we are doing more than 2 load directions but less than 12"
    )
    ...
    for i in np.arange(1, numElem + 1).reshape(-1):
        ...
        coordSys = "local"
        localFieldsTheta = extractFieldsThruThickness(
            plateStrainsTheta,
            meshData,
            blade.materials,
            blade.stacks,
        ...
        localFieldsThetaPlus90 = extractFieldsThruThickness(
            plateStrainsThetaPlus90,
            meshData,
            blade.materials,
            blade.stacks,
            blade.swstacks,
        ...
        nptsPerLayer = 2
        layer = __builtint__.sorted(
            np.array([np.arange(1, npts / nptsPerLayer + 1), np.arange(1, npts / nptsPerLayer + 1)])
        )
        for ix3 in np.arange(1, npts + 1).reshape(-1):
            matNumber = getattr(localFieldsTheta, (np.array(["element", num2str(elNo)]))).matNumber(
                ix3
            )
            if not len(blade.materials(matNumber).m) == 0:
                # Determine mean and amplitude stress
                MthetaFactor = (
                    getattr(localFieldsTheta, (np.array(["element", num2str(elNo)]))).sig11(ix3)
                    / Mtheta
                )
                MthetaPlus90Factor = (
                    getattr(localFieldsThetaPlus90, (np.array(["element", num2str(elNo)]))).sig11(
                        ix3
                    )
                    / MthetaPlus90
                )
                ...
                LthetaPlus90WithFactor[:, 1] = np.abs(
                    LthetaPlus90WithFactor[:, 0] * MthetaPlus90Factor
                )
                ...
                if isfinite(MthetaPlus90Factor):
                    if "Shifted Goodman" == IEC.fatigueCriterion:
                        layerFDthetaPlus90 = shiftedGoodman(
                            LthetaPlus90WithFactor, XTEN, XCMP, m, SFs, SFf
                        )
                else:
...
def getMomentMarkov(
    rccdata=None, wt=None, Yr=None, simtime=None, markovSize=None, chSpan=None, direction=None
):
    if chSpan == 1:
        baseStr = "Root"
    else:
    ...
    while not (
        contains(rccdata[ct, 1].label, baseStr)
        and contains(rccdata[ct, 1].label, "M")
        and contains(rccdata[ct, 1].label, direction)
    ):
        ct = ct + 1
    ...
    for w in np.arange(1, rccdata.shape[2 - 1] + 1).reshape(-1):
        ...
        if w == 1 or str(data.label) == str(rccdata[ct, w - 1].label):
            ...
            cycles = np.array(
                [[cycles], [data.cycles / simtime * (60 * 60 * 24 * 365.24 * Yr) * wt(w)]]
            )
        else:
            raise Exception(
                "Data channel name from current wind speed does not match the previous wind speed. Fatigue cycles cannot be summed."
            )
    ...
    markov[np.arange[2, end() + 1], 1] = 0.5 * np.transpose(
        (EDGESi(np.arange(1, end() - 1 + 1)) + EDGESi(np.arange(2, end() + 1)))
    )

    markov[1, np.arange[2, end() + 1]] = 0.5 * (
        EDGESj(np.arange(1, end() - 1 + 1)) + EDGESj(np.arange(2, end() + 1))
    )
...
def shiftedGoodman(markov, XTEN, XCMP, m, SFs, SFf):
    ...
    for i in range(num_range):
        for j in range(num_mean):
            ...
            n = markov(i + 1, j + 1)
            N = (
                (XTEN + np.abs(XCMP) - np.abs(2 * sm * SFs * FSloads - XTEN + np.abs(XCMP)))
                / (2 * sa * (SFf) * FSloads)
            ) ** m
            damage = damage + n / N
