            ...
            if interpMethod == "linear":
                ...
                for ndi in range(0, nbNds):
                    ...
                    iFun = interpolate.interp1d(
                        pDest, xDest, "cubic", axis=0, bounds_error=False, fill_value="extrapolate"
                    )
                    xAll = iFun(pAll)
                    xMat[ndi, :] = xAll
                    yDest = np.array(yDest)
                    iFun = interpolate.interp1d(
                        pDest, yDest, "cubic", axis=0, bounds_error=False, fill_value="extrapolate"
                    )
                    yAll = iFun(pAll)
                    yMat[ndi, :] = yAll
                if dimSpace == 3:
        ...
        for edi in range(0, self.numEdges):
            ...
            if el1 != -1 and el2 != -1:
                if elElim[el1] == 0 and elElim[el2] == 0:
                    if (
                        edLen > edgeFactor * elLongEdge[el1]
                        and edLen > edgeFactor * elLongEdge[el2]
                    ):
                        elElim[el1] = 1
                        elElim[el2] = 1
        ...
        while elsCreated:
            ...
            for edi in range(0, nEd):
                if self.edgeElements[edi, 1] == -1:
                    ...
                    projLen = (
                        0.2165 * edLen + 0.75 * self.avgProjLen
                    )  ## 0.25(sqrt(3)/2)edLen + 0.75avgProjLen
        ...
        fig = go.Figure(
            data=[
                go.Mesh3d(
                    ...
                    z=zLst,
                    colorbar_title="",
                    colorscale=[[0.0, "blue"], [0.5, "yellow"], [1.0, "red"]],
                    intensity=value,
                    intensitymode="cell",
                    i=v1,
                    j=v2,
