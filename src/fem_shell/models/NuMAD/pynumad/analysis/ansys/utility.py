        ...
        for kdp in range(stationIB.size - 1):
            if np.array(["single", "double"]) == stationIB.dptype[kdp]:
                ...
                if np.array(["flare", "hourglass"]) == stationIB.dptype[kdp]:
                    ...
                    skin_areas[kStation].startIB[end() + [np.arange[1, 2 + 1]]] = np.array(
                        [kdp, kdp]
                    )
                    skin_areas[kStation].endIB[end() + [np.arange[1, 2 + 1]]] = np.array(
                        [kdp, kdp + 1]
                    )
        ...
        for kdp in range(stationOB.dp.size - 1):
            if np.array(["single", "flare"]) == stationOB.dptype[kdp]:
                ...
                if np.array(["double", "hourglass"]) == stationOB.dptype[kdp]:
                    ...
                    # start and end of first area is current DP
                    # start and end of next area is current and next DP
                    skin_areas[kStation].startOB[end() + [np.arange[1, 2 + 1]]] = np.array(
                        [kdp, kdp]
                    )
                    skin_areas[kStation].endOB[end() + [np.arange[1, 2 + 1]]] = np.array(
                        [kdp, kdp + 1]
                    )
...
def postprocessANSYSfatigue(blade, meshData, wt, rccdata, IEC, loadsTable, config):
    if np.any(
        "all" in config.analysisFlags.fatigue.lower()
    ):  # NOTE probably need to workshop this -kb
        nSegments = 1
    else:
        ...
        for i in range(nSegments):
                    ...
                    for iWeb in range(nWebs):
                        ...
                        for iSpan in range(nSpanRegions):
                            elementList = np.array(
                                [elementList, meshData.shearWebElSets[iWeb][iSpan].elementList]
                            )
            ...
            for chSpan in range(nGage):
                ...
                if theta + 90 < 180:
                    ...
                    direction = str(theta - 90)
                LthetaPlus90 = getMomentMarkov(
                    rccdata, wt, Yr, simtime, markovSize, chSpan, direction
                )
                ...
                binnedElements = np.intersect(
                    np.where(plateStrainsThetaSet[:, 1] < z2),
                    np.where(plateStrainsThetaSet[:, 1] > z1),
                )
                fdData, plotFatigueChSpan = calcFatigue(
                    blade,
                    meshData,
                    IEC,
                    Ltheta,
                    ...
                    MthetaPlus90,
                    binnedElements,
                    plateStrainsThetaSet,
                    plateStrainsThetaPlus90Set,
                    iSegment,
                )
...
def getWindSpeedDistribution(avgws):
    ...
    if len(wt) != rccdata.shape[1]:
        raise Exception(
            "The number of Rayleigh weights is not equal to the number of wind speeds contained in the rccdata.mat file"
        )

    return wt, rccdata


def getLoadFactorsForElementsWithSameSection(
    ...
    def checkWrinkle(S_alphaBeta, mat, app, coreMatName):
        ...
        for kang in range(N):
            if str(matCore.type) == str("orthotropic"):
                Gc = 1 / (
                    np.sin(np.pi / 180 * angle) ** 2 * (1 / matCore.gyz)
                    + np.cos(np.pi / 180 * angle) ** 2 * (1 / matCore.gxz)
                )
