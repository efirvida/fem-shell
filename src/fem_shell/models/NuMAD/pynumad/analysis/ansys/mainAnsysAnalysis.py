...
def mainAnsysAnalysis(
    ...
    ansys_product = "ANSYS"
    if (
        ("imperfection" in analysisConfig["analysisFlags"])
        and analysisConfig["analysisFlags"]["imperfection"]
        and not analysisConfig["analysisFlags"]["globalBuckling"]
    ):
    ...
    for iLoad in range(len(loadsTable)):
        ...
        nodeData = np.concatenate(
            [np.arange(meshData["nodes"].shape[0]).reshape((-1, 1)), meshData["nodes"]], axis=1
        )
        ...
        if (
            "StaticNonlinear" in analysisConfig["analysisFlags"]
            and not len(analysisConfig["analysisFlags"].StaticNonlinear) == 0
            and analysisConfig["analysisFlags"].StaticNonlinear != 0
        ):
            fid.write("nlgeom,1\n" % ())
        ...
        if (
            iLoad == 0
            and "mass" in analysisConfig["analysisFlags"]
            and analysisConfig["analysisFlags"]["mass"] != 0
        ):
            # Get Mass Here
        ...
        if (
            "deflection" in analysisConfig["analysisFlags"]
            and analysisConfig["analysisFlags"]["deflection"] != 0
        ):
        ...
        if (
            "localBuckling" in analysisConfig["analysisFlags"]
            and not "imperfection" in analysisConfig["analysisFlags"]
            and not len(analysisConfig["analysisFlags"].imperfection) == 0
        ):
            # Check for wrinkling here in a linear analysis
            app, SkinAreas, compsInModel = writeAnsysGetFaceStresses(
                blade, fid, analysisConfig["analysisFlags"].localBuckling
            )
        ### Output resultant force and moments to file NOTE: Skip
        if (
            "resultantVSspan" in analysisConfig["analysisFlags"]
            and analysisConfig["analysisFlags"].resultantVSspan != 0
        ):
        ...
        if (
            "globalBuckling" in analysisConfig["analysisFlags"]
            and analysisConfig["analysisFlags"]["globalBuckling"] > 0
        ):
            ...
            if (
                ...
                raise Exception(
                    'analysisConfig["analysisFlags"].globalBuckling must be greater than or equal to zero'
                )
        fid.close()
        ...
        if (
            ...
            designvar["deflection"] = readAnsysDeflections(
                blade, analysisConfig, iLoad, deflectionFilename
            )
        ## ************************************************************************
        ...
        if (
            ...
            linearLoadFactors = readAnsysLinearBuckling(
                blade, analysisConfig, iLoad, fid, bucklingFilename
            )
        ## ************************************************************************
        ...
        if (
            ...
            for jj in range(len(imperfection)):
                for ii in range(len(linearLoadFactors)):
                    ...
                    nonlinearLoadFactors[ii, jj] = writeAnsysNonLinearBuckling(
                        ansysFilename,
                        ansysPath,
                        ansys_product,
                        analysisConfig,
                    ...
                    wrinklingLimitingElementData[ii, :, jj] = wrinklingForNonlinearBuckling(
                        blade,
                        analysisConfig["analysisFlags"].localBuckling,
                        settings,
                        ncpus,
                        ansysFilename,
                        ii,
        ...
        if (
            "localBuckling" in analysisConfig["analysisFlags"]
            and not len(analysisConfig["analysisFlags"].localBuckling) == 0
        ):
            if (
                "imperfection" in analysisConfig["analysisFlags"]
                and not len(analysisConfig["analysisFlags"].imperfection) == 0
                ...
                writeAnsysNonLinearLocalBuckling(
                    blade, analysisConfig, iLoad, fid, ansysFilename, ii, jj
                )
            # perform wrinkling check
            wrinklingLimitingElementData = writeAnsysFagerberWrinkling(
                app, SkinAreas, compsInModel, analysisConfig["analysisFlags"].localBuckling
            )
            designvar.localBuckling[iLoad] = wrinklingLimitingElementData[2]
            os.path.delete(
                "*faceAvgStresses.txt"
            )  # NOTE: I believe * is supposed to glob here, but I am not sure it is doing that -kb
        ## ************************************************************************
        # ================= READ FAILURE RESULTS INTO MATLAB =================
        if (
            "failure" in analysisConfig["analysisFlags"]
            and not len(analysisConfig["analysisFlags"]["failure"]) == 0
        ):
    ...
    if (
        "fatigue" in analysisConfig["analysisFlags"]
        and not len(analysisConfig["analysisFlags"]["fatigue"]) == 0
    ):
        if not len(varargin) == 0 and class_(varargin[0]) == "IECDef":
            ...
            # cd 'NuMAD'
            designvar.fatigue = postprocessANSYSfatigue(
                blade, meshData, wt, rccdata, IEC, loadsTable, analysisConfig
            )
        else:
...
def saveData(
    designvar=None, iLoad=None, airfoilSegmentName=None, iSpan=None, nodes=None, midNodei=None
):
