...
from pynumad.analysis.ansys.utility import (
    txt2mat,
    getLoadFactorsForElementsWithSameSection,
...
def writeAnsysDeflections(blade, config, iLoad, fid, deflectionFilename):
    ...
    for i in range(blade.ispan.size):
        ...
        fid.write(
            "*GET, node_num, NODE,0,NUM,MIN        !Get the smallest number node in the set\n" % ()
        )
        fid.write(
            "*DO, i, 1, NsectionNodes                 !loop through all nodes in cross section\n"
            % ()
        )
        ...
        fid.write(
            "node_num=NDNEXT(node_num)             !Get the next higher node number in the set\n"
            % ()
        )
        fid.write("*ENDDO\n" % ())
        fid.write("*CFCLOS\n" % ())
...
def writeAnsysFagerberWrinkling(app, SkinAreas, compsInModel, coreMatName):
    ...
    for kStation in range(TotalStations - 1):
        for kArea in range(SkinAreas[kStation].startIB.size):
            ...
            if coreMatName in mat.layer.layerName:
                ansysSecNumber = np.where(
                    str(SkinAreas[kStation].Material[kArea]) == str(compsInModel)
                )
                file = "section-" + str(ansysSecNumber) + "-faceAvgStresses.txt"
                avgFaceStress = txt2mat(file)
                os.delete(file)
                LF = getLoadFactorsForElementsWithSameSection(
                    LF, ansysSecNumber, avgFaceStress, app, mat, coreMatName
                )
    ...
    for kShearweb in range(TotalShearwebs):
        ...
        if coreMatName in mat.layer.layerName:
            ...
            os.delete(file)
            LF = getLoadFactorsForElementsWithSameSection(
                LF, ansysSecNumber + 1000, avgFaceStress, app, mat, coreMatName
            )
...
def writeAnsysNonLinearBuckling(
    ansysFilename, ansys_path, ansys_product, config, ii, jj, ncpus, iLoad
):
    ...
    ###### RUN ANSYS#######
    ansys_call = print(
        'SET KMP_STACKSIZE=2048k & "%s" -b -p %s -I %s -o %s -np %s',
        ansys_path,
        ansys_product,
        script_name,
        script_out,
...
def writeAnsysResultantVSSpan(blade, config, iLoad, fid):
    ...
    fid.write("elsize=%f\n" % (blade.mesh))
    fid.write(
        "nz=nint(%f/elsize) !Integer number of points to output resultant loads\n"
        % (blade.ispan(end()))
    )
    fid.write("zloc=0\n" % ())
    fid.write("delta=0.1\n" % ())
...
def writeAnsysRupture(config, iLoad, fid, failureFilename):
    ...
    fid.write("/BATCH  \n" % ())
    fid.write(
        "/COM,ANSYS RELEASE Release 18.1      BUILD 18.1      UP20170403       15:49:08\n" % ()
    )
    fid.write("/GRA,POWER\n " % ())
    fid.write("/GST,ON\n " % ())
...
def writeWrinklingForNonlinearBuckling(blade, coreMatName, settings, np, ansysFilename, i, j):
    ...
    wrinklingLimitingElementData = writeAnsysFagerberWrinkling(
        app, SkinAreas, compsInModel, coreMatName
    )
...
def writePlotFatigue(fname, plotFatigue):
    ...
    fid.write(
        "*dim,d_results,array,%i,2   !Need to specify the same size array as the data being read in\n"
        % (n)
    )
    ...
    for kStation in range(TotalStations):
        ...
        fid.write(
            ...
            % (
                ...
                blade.ispan[kStation],
                0,
                precurve_rot * 180 / np.pi,
                presweep_rot * 180 / np.pi,
            )
        )
    fid.write("\n   csys,0")
    fid.write("\nksel,all\n")
...
def write_ansys_loads(nodeData, loads, forcefilename, analysisConfig):
    ...
    maptype = "map3D_fxM0"
    if (
        ("FollowerForces" in analysisConfig["analysisFlags"])
        and not len(analysisConfig["analysisFlags"].FollowerForces) == 0
        and analysisConfig["analysisFlags"].FollowerForces != 0
        and ("StaticNonlinear" in analysisConfig["analysisFlags"])
