...
def writeBeamModel(wt_name, settings, blade, mu, log, directory="."):
    ...
    if "vabs" in settings["solver"].lower():
        ...
        if len(glob.glob(pattern)) == 0:
            raise RuntimeError(
                f"Could not find files with pattern: {pattern}. Beam model generation failed"
            )
            ...
            try:
                ...
                while not licenseAvailable and nLicenceTries <= MAXnLicenceTries - 1:
                    ...
                    if "Congratulations! No errors" in lines[-1]:
                        ...
                        raise Exception(
                            f"Cross-sectional homogenization for file {filePath} failed due to: \n {lines[-1]} \n Beam model creation failed."
                        )
    ...
    blade.idegreestwist = np.delete(
        blade.idegreestwist, -2
    )  # TEMP Need to delete the added station near tip
    ...
    for fileName in glob.glob(directory + "/" + wt_name + "*." + extension):
        ...
        beam_stiff[iStation, :, :], beam_inertia[iStation, :, :] = beamUtils.readVABShomogenization(
            fileName
        )
    ...
    if "beamdyn" in settings["beamSolver"].lower():
        ...
        propFileName = beamUtils.write_beamdyn_prop(
            directory, wt_name, radial_stations, beam_stiff, beam_inertia, mu
        )
