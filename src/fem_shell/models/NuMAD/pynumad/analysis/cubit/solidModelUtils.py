...
def makeOneVolume(
    ...
    if (
        "Station" + str(iStationEnd) in cubit.get_entity_name("surface", nextSurfaceID)
    ...
    if nCrossSections > 1:
        for iSpan in range(nCrossSections - 1):
            for partSurfaceIDs in range(nPartSurfaceIDs):
                ...
                spanwiseSplinesForAvolume, spanwiseSplines[partSurfaceIDs] = (
...
def makeBirdsMouth(blade, birdsMouthVerts, amplitudeFraction, iStationFirstWeb, iStationLastWeb):
    ...
    surfaceID = parse_cubit_list("surface", parseString)[
        0
