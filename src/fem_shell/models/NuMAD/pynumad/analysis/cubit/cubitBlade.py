...
def generateCubitCrossSections(
    blade, wt_name, settings, crosssectionParams, model2Dor3D, stationList=None, directory="."
):
    ...
    expandTEthicknesses = list(
        crosssectionParams["TE_adhesive"] + 6 * crosssectionParams["minimumLayerThickness"]
    )
...
def generateCubitSolidModel(blade, wt_name, settings, crosssectionParams, stationList=None):
    ...
    (
        ...
        materialsUsed,
        spanwiseMatOriCurve,
    ) = generateCubitCrossSections(blade, wt_name, settings, crosssectionParams, "3D", stationList)
