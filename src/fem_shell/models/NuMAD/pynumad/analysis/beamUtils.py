...
def write_beamdyn_axis(directory, wt_name, blade, radial_stations):
    ...
    file.write(
        " 50           load_retries    - Number of factored load retries before quitting the aimulation\n"
    )
    file.write(
    ...
    file.write(
        "         %u   kp_total        - Total number of key points (-) [must be at least 3]\n"
        % (n_pts)
    )
    file.write(
        "     1     %u                 - Member number; Number of key points in this member\n"
        % (n_pts)
    )
    ...
    file.write(
        '"%s"    BldFile - Name of file containing properties for blade (quoted string)\n'
    ...
    file.write(
        "3, 6, 9, 12, 15, 18, 21, 24, 27    OutNd          - Nodes whose values will be output  (-)\n"
    )
    file.write(
...
def runBeamDynStandAlone(beamDynDriverFileName, log, directory="."):
    ...
    try:
        this_cmd = (
            path_data["openFast"] + "beamdyn_driver " + directory + "/" + beamDynDriverFileName
        )
