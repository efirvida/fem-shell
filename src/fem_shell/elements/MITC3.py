...
class MITC3(ShellElement):
    ...
    **Assumed transverse shear strain field** uses 6 internal tying points:
    ...
    def __init__(
        ...
        # Local coordinate system (projected to 2D)
        ...
        # Legacy tying points (edge midpoints) - kept for compatibility
        self._tying_points_shear = [
            (0.5, 0.0),   # Edge 1-2 midpoint
            (0.5, 0.5),   # Edge 2-3 midpoint  
            (0.0, 0.5),   # Edge 1-3 midpoint
        ]
    ...
    def _get_dH(self, r: float, s: float) -> np.ndarray:
        ...
        dh_dx = J_inv[0, 0] * dh_dr + J_inv[1, 0] * dh_ds
        dh_dy = J_inv[0, 1] * dh_dr + J_inv[1, 1] * dh_ds
    ...
    def B_kappa(self, r: float, s: float) -> np.ndarray:
        ...
        Uses standard linear shape functions for bending curvatures.
        The bubble enrichment affects the element through the shear strains.
    ...
    def _evaluate_covariant_shear_at_point(self, r: float, s: float) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def B_gamma(self, r: float, s: float) -> np.ndarray:
        ...
        Implements the assumed transverse shear strain field from Lee, Lee & Bathe (2014)
        using internal tying points to prevent shear locking.
        
        The assumed strain field consists of:
        - Constant part: evaluated at tying points A, B, C
        - Linear part: evaluated at tying points D, E, F (reduces in-plane twisting)
        ...
        -------
        np.ndarray
            2×18 shear B matrix [γxz, γyz] (in local Cartesian coordinates)
        """
        # Get covariant shear strains at all tying points
        ...
        # =====================================================================
        # Constant part of assumed strain field (Eq. 15 from paper)
        # =====================================================================
        # e_rt_const = (2/3)*(e_rt_B - 0.5*e_st_B) + (1/3)*(e_rt_C + e_st_C)
        # e_st_const = (2/3)*(e_st_A - 0.5*e_rt_A) + (1/3)*(e_rt_C + e_st_C)
        ...
        # =====================================================================
        # Linear part of assumed strain field (Eq. 16 from paper)
        # =====================================================================
        # c_hat = (e_rt_F - e_rt_D) - (e_st_F - e_st_E)
        # e_rt_linear = (1/3) * c_hat * (3s - 1)
        # e_st_linear = (1/3) * c_hat * (1 - 3r)
        ...
        # =====================================================================
        # Complete assumed strain field (Eq. 17)
        # =====================================================================
        B_ert_assumed = B_ert_const + B_ert_linear
        B_est_assumed = B_est_const + B_est_linear
        
        # =====================================================================
        # Transform from covariant to Cartesian shear strains
        # =====================================================================
        # γxz = J_inv transforms the covariant strains to local Cartesian
        J_mat, _ = self.J(r, s)
        J_inv = np.linalg.inv(J_mat)
        
        # The transformation from (e_rt, e_st) to (γxz, γyz) involves the Jacobian
        # For a flat element: γxz ≈ (∂x/∂r)^-1 * e_rt, etc.
        # More precisely: [γxz, γyz]^T = J_inv^T @ [e_rt, e_st]^T
    ...
    def B_gamma_standard(self, r: float, s: float) -> np.ndarray:
        """
        Standard (non-MITC) transverse shear strain-displacement matrix.
        
        This is the displacement-based formulation prone to shear locking.
    ...
    def K(self) -> np.ndarray:
        ...
        """
        # Local stiffness
        K_local = self.k_m() + self.k_b() + self.k_s()
        ...
        # Use small fraction of average diagonal stiffness
        avg_diag = np.trace(K_global) / 18.0
        drill_stiff = 1e-6 * max(avg_diag, 1e-10)
        for i in range(3):
            idx = 6*i + 5  # θz
            K_global[idx, idx] += drill_stiff
    ...
    def update_configuration(self, displacements: np.ndarray) -> None:
        ...
        for i in range(3):
            u = displacements[6*i]
            v = displacements[6*i + 1]
            w = displacements[6*i + 2]
    ...
    def compute_tangent_stiffness(
        ...
        K_0 = self.k_m() + self.k_b() + self.k_s()
        ...
        # Get constitutive matrix (raw, without thickness)
        E = self.material.E
        nu = self.material.nu
        factor = E / (1 - nu**2)
        Cm_raw = factor * np.array([
            [1, nu, 0],
            [nu, 1, 0],
            [0, 0, (1 - nu) / 2]
        ])
        ...
        for (r, s), w in zip(self._gauss_points, self._gauss_weights):
            ...
            # Extract membrane parts (components 0, 1, 3: xx, yy, xy)
            B_m_L = B_L[[0, 1, 3], :]
            B_m_NL = B_NL[[0, 1, 3], :]
    ...
    def compute_internal_forces(
        ...
        for (r, s), w in zip(self._gauss_points, self._gauss_weights):
            ...
            if self.nonlinear:
                ...
                f_mem = B_total.T @ S_membrane * w * area * self.thickness
                f_int += f_mem
            else:
                ...
                mem_dofs = np.array([0, 1, 6, 7, 12, 13])
                u_mem = u_local[mem_dofs]
                eps_m = B_m[:, mem_dofs] @ u_mem
                sigma_m = Cm_raw @ eps_m
                f_int[mem_dofs] += B_m[:, mem_dofs].T @ sigma_m * w * area * self.thickness
            ...
            kappa = B_kappa[:, bend_dofs] @ u_bend
            M = Cb @ kappa  # Bending moments
            f_bend = B_kappa[:, bend_dofs].T @ M * w * area
            ...
            gamma = B_gamma[:, bend_dofs] @ u_bend
            Q = Cs @ gamma  # Shear forces
            f_shear = B_gamma[:, bend_dofs].T @ Q * w * area
    ...
    def compute_strain_energy(self) -> float:
        ...
        for (r, s), w in zip(self._gauss_points, self._gauss_weights):
            ...
            if self.nonlinear:
                ...
                mem_dofs = np.array([0, 1, 6, 7, 12, 13])
                u_mem = u_local[mem_dofs]
                E_mem = B_m[:, mem_dofs] @ u_mem
            
            U += 0.5 * E_mem @ Cm_raw @ E_mem * w * area * self.thickness
            ...
            u_bend = u_local[bend_dofs]
            kappa = B_kappa[:, bend_dofs] @ u_bend
            U += 0.5 * kappa @ Cb @ kappa * w * area
            
            # Shear strain energy
            B_gamma = self.B_gamma(r, s)
            gamma = B_gamma[:, bend_dofs] @ u_bend
            U += 0.5 * gamma @ Cs @ gamma * w * area
