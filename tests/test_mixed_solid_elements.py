...
class TestVolumeConservation:
    ...
    def test_tet_volume_accurate(self, steel):
        ...
        expected = 1.0 / 6.0
        assert np.isclose(volume, expected, rtol=1e-10), \
            f"Expected volume {expected}, got {volume}"
    ...
    def test_wedge_volume_accurate(self, steel):
        ...
        assert np.isclose(volume, 1.0, rtol=1e-10), \
            f"Expected volume ≈ 1.0 (scaled), got {volume}"
