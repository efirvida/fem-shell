...
def _compute_abd_matrices(
    ...
    """Compute A, B, D matrices and shear stiffness for a laminate.

    Returns A, B, D, Cs matrices (3x3 for A/B/D, 2x2 for Cs).
    """
    ...
    A = np.array([[A11, A12, 0], [A12, A22, 0], [0, 0, A66]])
    ...
    D11 = (
        2 * (Q11 * z[-1] ** 3 - Q11 * z[1] ** 3) / 3 + 2 * (Q22 * z[1] ** 3 - Q22 * z[-1] ** 3) / 3
    )
    D12 = (
        2 * (Q12 * z[-1] ** 3 - Q12 * z[1] ** 3) / 3 + 2 * (Q12 * z[1] ** 3 - Q12 * z[-1] ** 3) / 3
    )
    D22 = (
        2 * (Q22 * z[-1] ** 3 - Q22 * z[1] ** 3) / 3 + 2 * (Q11 * z[1] ** 3 - Q11 * z[-1] ** 3) / 3
    )
    D66 = (
        2 * (Q66 * z[-1] ** 3 - Q66 * z[1] ** 3) / 3 + 2 * (Q66 * z[1] ** 3 - Q66 * z[-1] ** 3) / 3
    )

    D = np.array([[D11, D12, 0], [D12, D22, 0], [0, 0, D66]])

    # Shear coupling Cs (2x2 for transverse shear)
    k_s = 5.0 / 6.0
    Cs = np.array([[k_s * G12 * h, 0.0], [0.0, k_s * G12 * h]])
...
def _write_ccx_inp(inp: Path, mesh: MeshModel, load: tuple[float, float, float]) -> None:
    ...
    lines.append(
        f"{E1:.8E},{E2:.8E},{E2:.8E},{nu12:.6f},{nu12:.6f},{0.0:.6f},{G12:.8E},{G12:.8E},{G12:.8E}"
    )
...
def test_composite_axial_tension(tmp_path: Path):
    ...
    A, B, D, Cs = _compute_abd_matrices([0, 90], thickness / 2, E1, E2, G12, nu12)
