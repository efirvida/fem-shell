...
def apply_load(mesh, solver, direction, total_load):
    ...
    free_nodes = sorted(mesh.get_node_set("free_edge").nodes.values(), key=lambda n: n.id)
...
class TestLinearStatic:
    ...
    def test_fx_in_plane(self):
        ...
        solver.add_dirichlet_conditions([DirichletCondition(get_clamped_dofs(mesh, dpn), 0.0)])
...
class TestNonlinearStatic:
    ...
    def test_large_displacement(self):
        ...
        solver_lin.add_dirichlet_conditions([DirichletCondition(get_clamped_dofs(mesh, dpn), 0.0)])
        ...
        solver_nl.add_dirichlet_conditions([DirichletCondition(get_clamped_dofs(mesh, dpn), 0.0)])
