...
def solid_element_setup(request, steel,
                         tetra4_nodes, tetra10_nodes,
                         wedge6_nodes, wedge15_nodes,
                         pyramid5_nodes, pyramid13_nodes,
                         hexa8_nodes, hexa20_nodes):
    ...
    def test_kronecker_delta_at_nodes(self, solid_element_setup):
        ...
        for node_idx, coords in nat_coords.items():
            ...
            for i, Ni in enumerate(N):
                if i == node_idx:
                    assert np.isclose(Ni, 1.0, atol=1e-8), \
                        f"N{i}({coords}) should be 1, got {Ni}"
...
class TestStiffnessMatrix:
    ...
    def test_stiffness_symmetry(self, solid_element_setup):
        ...
        assert max_asymmetry < 1e-10, \
            f"K not symmetric, max asymmetry: {max_asymmetry}"
    ...
    def test_positive_semi_definite(self, solid_element_setup):
        ...
        assert len(negative_eigvals) == 0, \
            f"Negative eigenvalues found: {negative_eigvals}"
...
class TestMassMatrix:
    ...
    def test_mass_symmetry(self, solid_element_setup):
        ...
        assert max_asymmetry < 1e-10, \
            f"M not symmetric, max asymmetry: {max_asymmetry}"
...
class TestAnalyticalTension:
    ...
    @pytest.mark.parametrize("element_class,nodes_fixture,n_nodes", [
        (HEXA8, "hexa8_nodes", 8),
...
class TestAnalyticalTorsion:
    ...
    def test_torsion_strain_pattern(self, steel, hexa8_nodes):
        ...
        assert abs(gamma_xz) > 1e-6 or abs(gamma_yz) > 1e-6, \
            "Torsion should produce shear strains"
...
class TestStressComputation:
    ...
    def test_von_mises_stress(self, steel, hexa8_nodes):
        ...
        assert np.isclose(vm, 1e6, rtol=0.001), \
            f"von Mises = {vm}, expected 1e6"

        # Hydrostatic stress (should give 0)
        stress_hydro = np.array([1e6, 1e6, 1e6, 0, 0, 0])
        vm_hydro = elem.compute_von_mises(stress_hydro)
        assert np.isclose(vm_hydro, 0, atol=1e-6), \
            f"Hydrostatic von Mises = {vm_hydro}, expected 0"
